...
final class StringUTF16 {
    ...
    /**
     * 对于字节序列的存储格式，目前有两大阵营，那就是Motorola的PowerPC ( RISC结构 )系列CPU和Intel的x86系列CPU。
     * PowerPC系列采用big endian方式存储数据，而x86系列则采用little endian方式存储数据。
     * Little-endian：将低序字节存储在起始地址（低位编址）（低位字节存入低地址，高位字节存入高地址，反序排列）
     * Big-endian：将高序字节存储在起始地址（高位编址）（低位字节存入高地址，高位字节存入低地址，依次排列）
     *
     *                  高位字--->低位字
     * 举个例子，如果我们将 0x1234abcd  写入到以0x0000开始的内存中，则结果为：
     ...
     *
     * 为什么要注意字节序的问题呢？你可能这么问。当然，如果你写的程序只在单机环境下面运行，
     * 并且不和别人的程序打交道，那么你完全可以忽略字节序的存在。
     *
     * @return
     */
    ...
    /**
     * 这是{@link Integer#getChars(int, int, byte[])}的变体，但适用于UTF16编码。
     *
     * @param i     转换的值
     * @param index 最低有效数字后的下一个索引
     * @param buf   目标buf，UTF16编码
