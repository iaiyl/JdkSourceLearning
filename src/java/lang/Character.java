...
/**
 * {@code Character}类是基本类型{@code char}的包装类型的对象。{@code Character}
 * 类型的对象包含{@code char}类型的单个字段。
 *
 * <p>此外，此类提供了一些方法来确定字符的类别(小写字母，数字等)，以及将字符从大写转换为
 * 小写，反之亦然。
 *
 * <p>字符信息基于 Unicode Standard, 10.0.0。
 *
 * <p>{@code Character}类的方法和数据由<i>UnicodeData</i>文件中的信息定义，该文件
 * 是Unicode Consortium维护的Unicode字符数据库的一部分。此文件指定各种属性，包括每个
 * 已定义的Unicode代码点或字符范围的名称和常规类别。
 *
 * <p>该文件及其描述可从Unicode Consortium获得：
 * <ul>
 ...
 * 
 * <p>{@code char}数据类型（即{@code Character}对象封装的值）基于原始Unicode规范，
 * 该规范将字符定义为固定宽度的 16 位实体。此后，Unicode标准进行了修改，允许表示需要
 * 超过16位的字符。合法的<em>代码点(code point)</em>s的范围现在是 U+0000 到 U+10FFF,
 * 称为<em>Unicode标量值(Unicode scalar value)</em>。
 * （请参阅Unicode标准中 U+<i>n</i>表示法的
 * <a href="http://www.unicode.org/reports/tr27/#notation"> <i>定义</i> </a>）。
 *
 * <p><a id="BMP"> 从 U+0000 到 U+FFFF 字符的集合 </a>有时被称为
 * <em>基本多文种平面(Basic Multilingual Plane , BMP)</em>。
 * 代码点大于 U+FFFF 的<a id="supplementary">Characters</a>称为
 * <em>补充字符 (supplementary character)</em>。Java平台使用{@code char}数组以及
 * {@code String}和{@code StringBuffer}类中的 UTF-16 表示。在此表示中，
 * 补充字符表示一对{@code char}值，第一个来自<em>高位替换</em>范围,(&#92;uD800-&#92;uDBFF)，
 * 第二个来自<em>低位替换</em>范围(&#92;uDC00-&#92;uDFFF)。
 *
 * <p>因此，一个{@code char}值表示基本多文种平面(BMP)代码点，包含替换代码点或
 * UTF-16编码的代码单元。{@code int}值代表全部的Unicode代码点，包括补充代码点。
 * {@code int}的低(最低有效) 21 位用于表示Unicode代码点，而高(最高有效) 11 位
 * 必须为零。除非另有说明，否则关于补充字符和替换{@code char}值的行为如下：
 * <ul>
 *     <li>
 *         仅接受{@code char}值的方法不支持补充字符。它们将替换范围中的{@code char}
 *         值视为未定义字符。例如，{@code Character.isLetter('\u005CuD840')}返回
 *         {@code false}, 即使此字符串中的任何低替换值后面的特定值也是表示字母。
 *     </li>
 *     <li>
 *         接受{@code int}值的方法支持所有的Unicode字符，包括补充字符。例如，
 *         {@code Character.isLetter(0x2F81A)}返回{@code true}，因为代码点值表示一个
 *         字母(CJK 表意文字)。
 *     </li>
 * </ul>
 *
 * <p>在 Java SE API 文档中，<em>Unicode代码点</em>用于 0+0000 到 U+10FFFF之间的字符，
 * <em>Unicode代码单元</em>用于 16 位 {@code char}值，这些值是 <em>UTF-16</em> 编码的代码单元。
 * 有关Unicode术语的更多信息，请参阅<a href="http://www.unicode.org/glossary">Unicode Glossary</a>。
 *
 *
 * @author  Lee Boynton
 * @author  Guy Steele
