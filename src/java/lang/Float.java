...
/**
 * {@code Float}类在对象中包装了基本类型{@code float}的值。类型为{@code Float}的对象
 * 包含一个类型为{@code float}的字段。
 *
 * <p>此外，这个类提供了一些方法，用于把{@code float} 转换为{@code String}，{@code String}
 * 转换为{@code float}，以及其他一些在处理{@code float}时有用的常量和方法。
 *
 * @author  Lee Boynton
...
public final class Float extends Number implements Comparable<Float> {
    /**
     * IEEE二进制浮点数算术标准（IEEE 754）是20世纪80年代以来最广泛使用的浮点数运算标准，
     * 为许多CPU与浮点运算器所采用。这个标准定义了表示浮点数的格式（包括负零-0）
     * 与反常值（denormal number），一些特殊数值（（无穷（Inf）与非数值（NaN）），
     * 以及这些数值的"浮点数运算符"；它也指明了四种数值舍入规则和五种例外状况
     * （包括例外发生的时机与处理方式）。
     *
     * IEEE 754规定了四种表示浮点数值的方式：单精确度（32位）、双精确度（64位）、
     * 延伸单精确度（43比特以上，很少使用）与延伸双精确度（79比特以上，通常以80位实现）。
     * 只有32位模式有强制要求，其他都是选择性的。大部分编程语言都提供了IEEE浮点数格式
     * 与算术，但有些将其列为非必需的。例如，IEEE 754问世之前就有的C语言，现在包括了
     * IEEE算术，但不算作强制要求（C语言的float通常是指IEEE单精确度，而double是指双精确度）。
     *
     * 一个浮点数 (Value) 的表示其实可以这样表示：
     *      Value = sign x exponent x fraction
     * 也就是浮点数的实际值(Value)，等于符号位(sign bit)乘以指数偏移值(exponent bias)
     * 再乘以分数值(fraction)。
     *
     * 本文表示比特的约定：把W个比特（bit）的数据，从内存地址低端到高端，以0到W-1编码。
     * 通常将内存地址低端的比特写在最右边，称作最低有效位（Least Significant Bit, LSB），
     * 代表最小的比特，改变时对整体数值影响最小的比特。声明这一点的必要性在于X86体系架构是小端序的数据存储。
     * 对于十进制整数N，必要时表示为N10以与二进制的数的表示N2相区分。
     * 对于一个数，其二进制科学计数法表示下的指数的值，下文称之为指数的实际值；
     * 而根据IEEE 754标准对指数部分的编码的值，称之为浮点数表示法指数域的编码值。
     *
     * 整体呈现：二进制浮点数是以符号数值表示法的格式存储——最高有效位被指定为符号位（sign bit）；
     * "指数部分"，即次高有效的e个比特，存储指数部分；最后剩下的f个低有效位的比特，存储"有效数"
     * （significand）的小数部分（在非规约形式下整数部分默认为0，其他情况下一律默认为1）。
     *
     * 指数偏移值：指数偏移值（exponent bias），即浮点数表示法中指数域的编码值，等于指数的实际值加
     * 上某个固定的值，IEEE 754标准规定该固定值为 2 ^(e-1) - 1，其的e为存储指数的比特的长度。
     * 以单精度浮点数为例，它的指数域是8个比特，固定偏移值是 2^(8-1) - 1 = 128 - 1 = 127。
     * 此为有号数的表示方式，单精度浮点数的指数部分实际取值是从-126到127（-127和128被用作特殊值处理，
     * 见下方"非规约形式的浮点数"和"特殊值"）。例如指数实际值为17 (10进制)，在单精度浮点数中的指数域编码值为
     * 144 (10进制)
     */

    /**
     * 持有{@code float}类型正无穷的常量。
     * 它等于{@code Float.intBitsToFloat(0x7f800000)}返回的值。
     */
    public static final float POSITIVE_INFINITY = 1.0f / 0.0f;

    /**
     * 持有{@code float}类型负无穷的常量。
     * 它等于{@code Float.intBitsToFloat(0xff800000)}返回的值。
     */
    public static final float NEGATIVE_INFINITY = -1.0f / 0.0f;

    /**
     * 持有{@code float}类型的 Not-a-Number(NaN) 的常量。
     * 它等于{@code Float.intBitsToFloat(0x7fc00000)}返回的值。
     */
    public static final float NaN = 0.0f / 0.0f;

    /**
     * 持有{@code float}类型的 最大正有限数 的常量，
     * (2-2<sup>-23</sup>)&middot;2<sup>127</sup>。
     * 它等于十六进制浮点文字 {@code 0x1.fffffeP+127f},
